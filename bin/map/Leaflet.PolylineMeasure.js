...
(function (factory) {
    ...
    let _measureControlId = 'polyline-measure-control';
    let _unicodeClass = 'polyline-measure-unicode-icon';
    ...
    L.Control.PolylineMeasure = L.Control.extend({
        ...
        options: {
            ...
            position: 'topleft',
            /**
             * Which units the distances are displayed in. Possible values are: 'metres', 'kilometres', 'landmiles', 'nauticalmiles'
             * @type {String}
             * @default
             */
            unit: 'metres',
        ...
        onAdd: function (map) {
            ...
            let title = this.options.measureControlTitleOn;
            let label = this.options.measureControlLabel;
            let classes = this.options.measureControlClasses;
            if (label.indexOf('&') !== -1) {
            ...
            if (this.options.showClearControl) {
                title = this.options.clearControlTitle;
                label = this.options.clearControlLabel;
                classes = this.options.clearControlClasses;
                if (label.indexOf('&') !== -1) {
                    classes.push(_unicodeClass);
                }
            ...
            if (this.options.showUnitControl) {
                if (this.options.unit === "metres") {
                    label = this.options.unitControlLabel.metres;
                    title = this.options.unitControlTitle.text + " [" + this.options.unitControlTitle.metres + "]";
                }
                else if (this.options.unit === "kilometres") {
                    label = this.options.unitControlLabel.kilometres;
                    title = this.options.unitControlTitle.text + " [" + this.options.unitControlTitle.kilometres + "]";
                } else if (this.options.unit === "landmiles") {
                    label = this.options.unitControlLabel.landmiles;
                    title = this.options.unitControlTitle.text + " [" + this.options.unitControlTitle.landmiles + "]";
                } else {
                    label = this.options.unitControlLabel.nauticalmiles;
                    title = this.options.unitControlTitle.text + " [" + this.options.unitControlTitle.nauticalmiles + "]";
                }
                classes = [];
        ...
        _changeUnit: function () {
            ...
            this._arrPolylines.map(function (line) {
                ...
                line.circleCoords.map(function (point, point_index) {
                    if (point_index >= 1) {
                        let distance = line.circleCoords [point_index - 1].distanceTo(line.circleCoords [point_index]);
            ...
            if (e.keyCode === 27) {
                ...
                if (this._resumeFirstpointFlag === true) {
                    ...
                    if (this.options.showBearings === true) {
                        text += this.options.bearingTextIn + ':--- deg<br>' + this.options.bearingTextOut + ':--- deg';
                    }
                    text += '<div class="polyline-measure-tooltip-difference">+' + '0</div>';
                    text += '<div class="polyline-measure-tooltip-total">' + '0</div>';
                    this._arrPolylines[lineNr].tooltips [0]._icon.innerHTML = text;
                    this._arrPolylines[lineNr].tooltips.map(function (item, index) {
                        if (index >= 1) {
                            let distance = this._arrPolylines[lineNr].circleCoords[index - 1].distanceTo(this._arrPolylines[lineNr].circleCoords[index]);
                            let lastCircleCoords = this._arrPolylines[lineNr].circleCoords[index - 1];
                            let mouseCoords = this._arrPolylines[lineNr].circleCoords[index];
                            totalDistance += distance;
                            let prevTooltip = this._arrPolylines[lineNr].tooltips[index - 1]
        ...
        _polylineArc: function (_from, _to) {
            function _GCinterpolate(f) {
                ...
                let x = A * Math.cos(fromLat) * Math.cos(fromLng) + B * Math.cos(toLat) * Math.cos(toLng);
                let y = A * Math.cos(fromLat) * Math.sin(fromLng) + B * Math.cos(toLat) * Math.sin(toLng);
                let z = A * Math.sin(fromLat) + B * Math.sin(toLat);
                // atan2 better than atan-function cause results are from -pi to +pi
                // => results of latInterpol, lngInterpol always within range -180deg ... +180deg  => conversion into values < -180deg or > + 180deg has to be done afterwards
                let latInterpol = 180 / Math.PI * Math.atan2(z, Math.sqrt(Math.pow(x, 2) + Math.pow(y, 2)));
                let lngInterpol = 180 / Math.PI * Math.atan2(y, x);
                // don't split polyline if it crosses dateline ( -180deg or +180deg).  Without the polyline would look like: +179deg ==> +180deg ==> -180deg ==> -179deg...
                // algo: if difference lngInterpol-from.lng is > 180deg there's been an unwanted split from +180 to -180 cause an arc can never span >180deg
            ...
            let fromLat = _from.lat;  // work with with copies of object's elements _from.lat and _from.lng, otherwise they would get modiefied due to call-by-reference on Objects in Javascript
            ...
            let d = 2.0 * Math.asin(Math.sqrt(Math.pow(Math.sin((fromLat - toLat) / 2.0), 2) + Math.cos(fromLat) * Math.cos(toLat) * Math.pow(Math.sin((fromLng - toLng) / 2.0), 2)));
            let arrLatLngs;
            if (d === 0) {
        ...
        _updateTooltip: function (currentTooltip, prevTooltip, total, difference, lastCircleCoords, mouseCoords) {
            ...
            let calcAngle = function (p1, p2, direction) {
            ...
            let angleIn = calcAngle(mouseCoords, lastCircleCoords, "inbound");
            let angleOut = calcAngle(lastCircleCoords, mouseCoords, "outbound");
            let totalRound = this._getDistance(total);
            let differenceRound = this._getDistance(difference);
            // disable to avoid lot of calls
            // const latlngalt = getElevation(mouseCoords)
            // let textCurrent = `${mouseCoords.lat.toFixed(7)}, ${mouseCoords.lng.toFixed(7)}<br>`;
            let textCurrent = `${mouseCoords.lat.toFixed(7)}, ${mouseCoords.lng.toFixed(7)}<br>`;
            textCurrent += `Altitude: ---m<br>`;
            if (differenceRound.value > 0) {
                if (this.options.showBearings === true) {
                    textCurrent += this.options.bearingTextIn + ': ' + angleIn + ' deg<br>' + this.options.bearingTextOut + ':--- deg';
                }
            ...
            if ((this.options.showBearings === true) && (prevTooltip)) {
                let textPrev = prevTooltip._icon.innerHTML;
                let regExp = new RegExp(this.options.bearingTextOut + '.* deg');
                prevTooltip._icon.innerHTML = textPrev.replace(regExp, this.options.bearingTextOut + ': ' + angleOut + " deg");
            }
        },
        ...
        _drawArrow: function (arcLine) {
            ...
            let center = [P1[0] + diffLat12 / 2, P1[1] + diffLng12 / 2];  // center of Great-circle distance, NOT of the arc on a Mercator map! reason: a) to complicated b) map not always Mercator c) good optical feature to see where real center of distance is not the "virtual" warped arc center due to Mercator projection
            ...
            let newArrowMarker = L.marker(center, {icon: iconArrow, zIndexOffset: -50}).addTo(this._layerPaint);  // zIndexOffset to draw arrows below tooltips
        ...
        _mouseMove: function (e) {
            ...
            this._map.on('click', this._mouseClick, this);  // necessary for _dragCircle. If switched on already within _dragCircle an unwanted click is fired at the end of the drag.
            if (!mouseCoords || !this._currentLine) {
                return;
            }
            let lastCircleCoords = this._currentLine.circleCoords.last();
        ...
        _startLine: function (clickCoords) {
            ...
            let polylineState = this;   // use "polylineState" instead of "this" to allow measuring on 2 different maps the same time
            ...
            this._currentLine = {
                ...
                handleMarkers: function (latlng) {
                    ...
                    let lastCircleMarker = this.circleMarkers.last();
                    if (lastCircleMarker) {
                    ...
                    let newCircleMarker = new L.CircleMarker(latlng, polylineState.options.currentCircle).addTo(polylineState._layerPaint);
                    newCircleMarker.bindTooltip(polylineState.options.tooltipTextFinish + polylineState.options.tooltipTextDelete, {
                        direction: 'top',
                        opacity: 0.7,
                        className: 'polyline-measure-popupTooltip'
                    });
                ...
                addPoint: function (mouseCoords) {
                    let lastCircleCoords = this.circleCoords.last();
                    ...
                    if (this.circleCoords.length > 1) {
                        let arc = polylineState._polylineArc(lastCircleCoords, mouseCoords);
                        ...
                        let arrowMarker = polylineState._drawArrow(arc);
                        ...
                        let distanceSegment = lastCircleCoords.distanceTo(mouseCoords);
                        this.distance += distanceSegment;
                        currentTooltip = polylineState._currentLine.tooltips.last();
                        let prevTooltip = polylineState._currentLine.tooltips.slice(-1, -2)[0];
                    ...
                    if (currentTooltip) {
                        let latlngalt = getElevation(mouseCoords)
                        let tooltipText = currentTooltip._icon.innerHTML;
                        let regExp = new RegExp('Altitude.*m<br>');
                        currentTooltip._icon.innerHTML = tooltipText.replace(regExp, 'Altitude: ' + latlngalt.alt + "m<br>");
                        map.fire('polylinemeasure:new-point', {index: this.circleCoords.length - 1, latlngalt});
                        currentTooltip.setLatLng(mouseCoords);
                    }
                    // add new tooltip to update on mousemove
                    let tooltipNew = this.getNewToolTip(mouseCoords);
                ...
                finalize: function () {
                    ...
                    if (this.circleCoords.length > 1) {
                        ...
                        polylineState._currentLine.circleMarkers.map(function (circle) {
                            circle.bindTooltip(polylineState.options.tooltipTextMove + polylineState.options.tooltipTextDelete, {
                                direction: 'top',
                                opacity: 0.7,
                                className: 'polyline-measure-popupTooltip'
                            })
                        });
                        polylineState._currentLine.circleMarkers[0].bindTooltip(polylineState.options.tooltipTextMove + polylineState.options.tooltipTextDelete + polylineState.options.tooltipTextResume, {
                            direction: 'top',
                            opacity: 0.7,
                            className: 'polyline-measure-popupTooltip'
                        });
                        lastCircleMarker.bindTooltip(polylineState.options.tooltipTextMove + polylineState.options.tooltipTextDelete + polylineState.options.tooltipTextResume, {
                            direction: 'top',
                            opacity: 0.7,
                            className: 'polyline-measure-popupTooltip'
                        });
            ...
            let firstTooltip = L.marker(clickCoords, {
                icon: icon,
                interactive: false
            })
            firstTooltip.addTo(this._layerPaint);

            let latlngalt = getElevation(clickCoords)
            map.fire('polylinemeasure:new-point', {index: 0, latlngalt});

            let text = `${clickCoords.lat.toFixed(7)}, ${clickCoords.lng.toFixed(7)}<br>`;
            text += `Altitude: ${latlngalt.alt}m<br>`;
            if (this.options.showBearings === true) {
                text += this.options.bearingTextIn + ':--- deg<br>' + this.options.bearingTextOut + ':--- deg';
            }
            text += '<div class="polyline-measure-tooltip-difference">+' + '0</div>';
        ...
        _resumePolylinePath: function (e) {
            if (e.originalEvent.ctrlKey === true) {    // just resume if user pressed the CTRL-Key while clicking onto the last circle
                ...
                let tooltipNew = this._currentLine.getNewToolTip(e.latlng);
                ...
                this._currentLine.circleMarkers.last().bindTooltip(this.options.tooltipTextMove + this.options.tooltipTextDelete, {
                    direction: 'top',
                    opacity: 0.7,
                    className: 'polyline-measure-popupTooltip'
                });
        ...
        _clickedArrow: function (e) {
            if (e.originalEvent.ctrlKey) {
                let lineNr = e.target.cntLine;
                let arrowNr = e.target.cntArrow;
                this._arrPolylines[lineNr].arrowMarkers [arrowNr].removeFrom(this._layerPaint);
                let newCircleMarker = new L.CircleMarker(e.latlng, this.options.intermedCircle).addTo(this._layerPaint);
                newCircleMarker.cntLine = lineNr;
                newCircleMarker.on('mousedown', this._dragCircle, this);
                newCircleMarker.bindTooltip(this.options.tooltipTextMove + this.options.tooltipTextDelete, {
                    direction: 'top',
                    opacity: 0.7,
                    className: 'polyline-measure-popupTooltip'
                });
                ...
                let arc1 = this._polylineArc(this._arrPolylines[lineNr].circleCoords[arrowNr], e.latlng);
                arc1.pop();
                let arc2 = this._polylineArc(e.latlng, this._arrPolylines[lineNr].circleCoords[arrowNr + 2]);
                ...
                let arrowMarker = this._drawArrow(arc1);
        ...
        _updateTooltipAltitude: function(tooltip, latlng) {
            let latlngalt = getElevation(latlng)
            let tooltipText = tooltip._icon.innerHTML;
            let regExp = new RegExp('Altitude.*m<br>');
            tooltip._icon.innerHTML = tooltipText.replace(regExp, 'Altitude: ' + latlngalt.alt + "m<br>");
        },
        ...
        _dragCircleMouseup: function () {
            ...
            if ((this._circleNr === 0) || (this._circleNr === this._arrPolylines[this._lineNr].circleCoords.length - 1)) {
                this._e1.target.bindTooltip(this.options.tooltipTextMove + this.options.tooltipTextDelete + this.options.tooltipTextResume, {
                    direction: 'top',
                    opacity: 0.7,
                    className: 'polyline-measure-popupTooltip'
                });
            } else {
                this._e1.target.bindTooltip(this.options.tooltipTextMove + this.options.tooltipTextDelete, {
                    direction: 'top',
                    opacity: 0.7,
                    className: 'polyline-measure-popupTooltip'
                });
            ...
            /// todo
            this._updateTooltipAltitude(this._arrPolylines[this._lineNr].tooltips[this._circleNr], this._e1.latlng)
        ...
        _dragCircleMousemove: function (e2) {
            let mouseNewLat = e2.latlng.lat;
            let mouseNewLng = e2.latlng.lng;
            let latDifference = mouseNewLat - this._mouseStartingLat;
            let lngDifference = mouseNewLng - this._mouseStartingLng;
            let currentCircleCoords = L.latLng(this._circleStartingLat + latDifference, this._circleStartingLng + lngDifference);
            let arcpoints = this._arcpoints;
            let lineNr = this._e1.target.cntLine;
            this._lineNr = lineNr;
            let circleNr = this._e1.target.cntCircle;
            ...
            let lineCoords = this._arrPolylines[lineNr].polylinePath.getLatLngs(); // get Coords of each Point of the current Polyline
            if (circleNr >= 1) {   // redraw previous arc just if circle is not starting circle of polyline
                let newLineSegment1 = this._polylineArc(this._arrPolylines[lineNr].circleCoords[circleNr - 1], currentCircleCoords);
                ...
                let arrowMarker = this._drawArrow(newLineSegment1);
            ...
            if (circleNr < this._arrPolylines[lineNr].circleCoords.length - 1) {   // redraw following arc just if circle is not end circle of polyline
                let newLineSegment2 = this._polylineArc(currentCircleCoords, this._arrPolylines[lineNr].circleCoords[circleNr + 1]);
        ...
        _resumeFirstpointMousemove: function (e) {
            ...
            let totalDistance = 0;
            let distance = mouseCoords.distanceTo(this._arrPolylines[lineNr].circleCoords[0]);
            let lastCircleCoords = mouseCoords;
            let currentCoords = this._arrPolylines[lineNr].circleCoords[0];
            totalDistance += distance;
            let prevTooltip = this._tooltipNew;
            let currentTooltip = this._arrPolylines[lineNr].tooltips[0]
        ...
        _resumeFirstpointClick: function (e) {
            ...
            this._arrPolylines[lineNr].circleMarkers [0].bindTooltip(this.options.tooltipTextMove + this.options.tooltipTextDelete, {
                direction: 'top',
                opacity: 0.7,
                className: 'polyline-measure-popupTooltip'
            });
            let newCircleMarker = new L.CircleMarker(e.latlng, this.options.startCircle).addTo(this._layerPaint);
            newCircleMarker.cntLine = lineNr;
            newCircleMarker.cntCircle = 0;
            newCircleMarker.on('mousedown', this._dragCircle, this);
            newCircleMarker.bindTooltip(this.options.tooltipTextMove + this.options.tooltipTextDelete + this.options.tooltipTextResume, {
                direction: 'top',
                opacity: 0.7,
                className: 'polyline-measure-popupTooltip'
            });
            ...
            let arc = this._polylineArc(e.latlng, currentCircleCoords);
            let arrowMarker = this._drawArrow(arc);
        ...
        _dragCircle: function (e1) {
            ...
            if (e1.originalEvent.shiftKey) {    // it's not possible to use "ALT-Key" instead, cause this won't work in some Linux distributions (there it's the default hotkey for moving windows)
                ...
                if (this._arrPolylines[lineNr]) {
                    ...
                    let lineCoords = this._arrPolylines[lineNr].polylinePath.getLatLngs();
                    ...
                    if (circleNr === 0) {
                        ...
                        this._arrPolylines[lineNr].circleMarkers [0].bindTooltip(this.options.tooltipTextMove + this.options.tooltipTextDelete + this.options.tooltipTextResume, {
                            direction: 'top',
                            opacity: 0.7,
                            className: 'polyline-measure-popupTooltip'
                        });
                        ...
                        this._arrPolylines[lineNr].circleMarkers [circleNr - 1].bindTooltip(this.options.tooltipTextMove + this.options.tooltipTextDelete + this.options.tooltipTextResume, {
                            direction: 'top',
                            opacity: 0.7,
                            className: 'polyline-measure-popupTooltip'
                        });
                        ...
                        this._arrPolylines[lineNr].arrowMarkers [circleNr - 1].removeFrom(this._layerPaint);
                        this._arrPolylines[lineNr].arrowMarkers [circleNr].removeFrom(this._layerPaint);
                        let arrowMarker = this._drawArrow(newLineSegment);
                    ...
                    if (circleNr === 0) {
                        ...
                        lineCoords.splice(0, arcpoints - 1);
                        this._currentLine.circleMarkers [0].bindTooltip(this.options.tooltipTextMove + this.options.tooltipTextDelete + this.options.tooltipTextResume, {
                            direction: 'top',
                            opacity: 0.7,
                            className: 'polyline-measure-popupTooltip'
                        });
                        ...
                        this._currentLine.circleMarkers [circleNr - 1].bindTooltip(this.options.tooltipTextMove + this.options.tooltipTextDelete + this.options.tooltipTextResume, {
                            direction: 'top',
                            opacity: 0.7,
                            className: 'polyline-measure-popupTooltip'
                        });
                    ...
                    this._currentLine.tooltips.map(function (item, index, arr) {
                        if (index >= 1) {
                            let distance, mouseCoords;
                            let prevTooltip = this._currentLine.tooltips[index - 1];
                            let lastCircleCoords = this._currentLine.circleCoords[index - 1];
